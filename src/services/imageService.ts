...
export const ImageService = {
    async generateImage(prompt: string, visualStyle: string = '', highRes: boolean = false, hierarchy: ImageModelID[] = ['gemini-3-pro-image-preview', 'gemini-2.5-flash-image', 'imagen-3.0-generate-001']): Promise<string> {
        const client = LLMClient.getInstance().getClient();
    ...
    async editImage(base64Image: string, prompt: string, hierarchy: ImageModelID[] = ['gemini-3-pro-image-preview', 'gemini-2.5-flash-image']): Promise<string> {
        const client = LLMClient.getInstance().getClient();
